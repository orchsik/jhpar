...
export const solution = (N: number, ropeList: number[]) => {
  ropeList.sort((a, b) => b - a);
  ropeList = ropeList.map((rope, i) => rope * (i + 1));
  ropeList.sort((a, b) => b - a);
  return ropeList[0];
};

const ropeList: number[] = [10, 15];
const N = ropeList.length;
const R = solution(N, ropeList);
console.log(R);

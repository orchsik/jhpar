...
// 항상 물을 막을 때 적어도 그 위치의 좌우 0.5만큼 간격을 줘야 물이 다시는 안 샌다고 생각한다.
...
export const solution = (inputList: number[][]) => {
  const [N, L] = inputList[0];
  const leakPositionList = inputList[1];
  leakPositionList.sort((a, b) => a - b);

  let tapePositionList: number[] = [leakPositionList[0]];
  let tapeScope = leakPositionList[0] - 0.5 + L;
  leakPositionList.forEach((position, i) => {
    if (position + 0.5 > tapeScope) {
      tapePositionList.push(position);
      tapeScope = position - 0.5 + L;
    }
  });
  return tapePositionList.length;
};
